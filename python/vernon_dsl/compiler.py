...
_SCALARS = {"bool": "i1", "i32": "i32", "u32": "i32", "f16": "f16", "f32": "f32", "f64": "f64"}
...
class DslType:
    ...
    def is_float(self) -> bool:
        ...
        return self.kind == "tensor" and isinstance(self.arguments[0], DslType) and self.arguments[0].is_float
    ...
    def is_integer(self) -> bool:
        ...
        return self.kind == "tensor" and isinstance(self.arguments[0], DslType) and self.arguments[0].is_integer
...
class FrontendCompileRequest:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "captured_constants", tuple(sorted(self.captured_constants, key=lambda value: value[0]))
        )
...
@dataclass(frozen=True)
class _GeneratedInterfacePlan:
    implicit_samplers: tuple[_ImplicitSamplerPlan, ...]
...
def _plan_generated_interface(
    context: _ModuleContext,
    node: ast.FunctionDef,
    argument_annotations: list[AnnotatedType],
    stage: str | None,
) -> _GeneratedInterfacePlan:
    argument_types = {
    ...
    for value in ast.walk(node):
        ...
        if texture_name is None or argument_types.get(texture_name, DslType("void", "void")).kind != "texture":
            continue
    ...
    sampler_plans: list[_ImplicitSamplerPlan] = []
    for texture_name in sorted(implicit_textures):
        ...
        sampler_plans.append(_ImplicitSamplerPlan(texture_name, descriptor_set, binding))

    return _GeneratedInterfacePlan(
        tuple(sampler_plans),
...
class _TypeParser:
    ...
    def parse(self, node: ast.AST) -> AnnotatedType:
        ...
        if isinstance(node, ast.Subscript) and (_name(node.value) or "").split(".")[-1] == "Annotated":
            ...
            return AnnotatedType(base, tuple(self._metadata(item) for item in items[1:]))
    ...
    def parse_type(self, node: ast.AST) -> DslType:
        ...
        if constructor in {"vec", "mat"}:
            ...
            if len(items) != dimensions + 1:
                raise self.context.error(
                    node, f"{constructor} requires {dimensions} dimension(s) followed by an element type"
                )
            shape = tuple(self._positive_int(item, f"{constructor} dimension") for item in items[:dimensions])
            return DslType("tensor", "Tensor", (self.parse_type(items[-1]), *shape))
        if constructor in {"vec2", "vec3", "vec4", "mat2", "mat3", "mat4"}:
            ...
            return DslType("tensor", "Tensor", (self.parse_type(items[0]), *shape))
        if constructor == "Array":
            if len(items) != 2:
                raise self.context.error(node, "Array requires an element type and fixed size")
            return DslType("array", "Array", (self.parse_type(items[0]), self._positive_int(items[1], "array size")))
        if constructor == "Buffer":
            ...
            arguments: tuple[DslType | int | str, ...] = (self.parse_type(items[0]),)
            if len(items) == 2:
...
class _FunctionEmitter:
    def __init__(
        ...
        self.interface_plan = _plan_generated_interface(context, node, argument_annotations, stage)
        self.returned = False
    ...
    def emit(self) -> list[str]:
        ...
        for index, (argument, annotation) in enumerate(
            ...
            has_builtin = any(item.kind == "builtin" for item in annotation.metadata)
            if self.stage == "compute" and value_type.kind == "tensor" and not has_builtin:
                value_type = DslType(
                    "addressable_tensor",
                    "Tensor",
    ...
    def _statement(self, node: ast.stmt) -> None:
        ...
        if isinstance(node, ast.Return):
            if node.value is None:
                ...
                self._require_same_type(node.value, self.signature.result, value.type)
                entry_fields = self._entry_result_fields()
                if entry_fields is not None:
    ...
    def _if(self, node: ast.If) -> None:
        ...
        assigned = self._assigned_names((*node.body, *node.orelse)) & self.environment.keys()
        ...
        for statement in node.body:
            if isinstance(statement, ast.Return):
                raise self.context.error(statement, "return inside if is not supported in the first frontend stage")
            self._statement(statement)
    ...
    def _for(self, node: ast.For) -> None:
        ...
        for statement in node.body:
            if isinstance(statement, ast.Return):
                raise self.context.error(statement, "return inside for is not supported in the first frontend stage")
            self._statement(statement)
        self._line("scf.yield")
        self.indent -= 1
        self._line("}")
    ...
    def _compare(self, node: ast.Compare) -> Value:
        ...
        if left.type.kind == "tensor":
            result_type = DslType("tensor", "Tensor", (result_type, *left.type.arguments[1:]))
    ...
    def _call(self, node: ast.Call) -> Value:
        ...
        if name == "matmul":
            ...
            if left.type.kind != "tensor" or len(left.type.arguments) != 3:
                raise self.context.error(node, "matmul left operand must be a matrix")
            ...
            elif right.type.kind == "tensor" and len(right.type.arguments) == 3 and right.type.arguments[1] == columns:
                result_type = DslType("tensor", "Tensor", (element, rows, right.type.arguments[2]))
            else:
                raise self.context.error(node, "matmul operands have incompatible shapes")
            return self._intrinsic(node, name, arguments, result_type)
        if name == "texture_sample":
            ...
            if lod is not None:
                operands.append(lod)
            return self._intrinsic(node, name, operands, DslType("tensor", "Tensor", (element, 4)))
        if name == "texture_size":
            ...
            return self._intrinsic(
                node, name, arguments, DslType("tensor", "Tensor", (DslType("scalar", "u32"), result_rank))
            )
        ...
        for argument, expected in zip(arguments, signature.arguments, strict=True):
    ...
    def _cast(self, node: ast.AST, value: Value, target: DslType) -> Value:
        ...
        if value.type.kind == "index":
            ...
            operation = "arith.uitofp" if value.type.name == "u32" else "arith.sitofp"
        elif value.type.is_float and target.is_integer:
            operation = "arith.fptoui" if target.name == "u32" else "arith.fptosi"
    ...
    def _index(self, node: ast.Subscript) -> Value:
        ...
        if value.type.kind in {"buffer", "addressable_tensor"}:
            index = (
                ...
                if value.type.kind == "addressable_tensor"
    ...
    def _store_index(self, target: ast.Subscript, value: Value) -> None:
        ...
        index = (
            ...
            if buffer.type.kind == "addressable_tensor"
...
class Compiler:
    ...
    @staticmethod
    def _collect_structs(module: ast.Module, context: _ModuleContext, parser: _TypeParser) -> None:
        for node in module.body:
    ...
    @staticmethod
    def _collect_signatures(module: ast.Module, context: _ModuleContext, parser: _TypeParser) -> None:
        for node in module.body:
            ...
            for argument in node.args.args:
                ...
                if shared and (
                    annotation.metadata
                    or annotation.type.kind in {"addressable_tensor", "buffer", "sampler", "texture"}
                ):
            ...
            if node.returns is not None and not (isinstance(node.returns, ast.Constant) and node.returns.value is None):
                ...
                if shared and (
                    result_annotation.metadata or result.kind in {"addressable_tensor", "buffer", "sampler", "texture"}
                ):
    ...
    def _decorator(node: ast.FunctionDef, context: _ModuleContext) -> tuple[str | None, tuple[int, int, int] | None]:
        ...
        for decorator in node.decorator_list:
            ...
            elif isinstance(decorator, ast.Call) and (_name(decorator.func) or "").split(".")[-1] in {
                "compute",
                "kernel",
            }:
