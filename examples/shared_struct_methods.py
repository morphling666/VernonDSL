...
class Light:
    ...
    color: vd.vec3[vd.f32]
    intensity: vd.f32

    @vd.func(shared=True)
    def distance_to(self, point: vd.vec3[vd.f32]) -> vd.f32:
        return vd.norm(self.position - point)

    @vd.func(shared=True)
    def contribution(self, point: vd.vec3[vd.f32]) -> vd.vec3[vd.f32]:
    ...
    def device_lambert(
        self,
        point: vd.vec3[vd.f32],
        normal: vd.vec3[vd.f32],
    ) -> vd.vec3[vd.f32]:
...
def main() -> None:
    ...
    light = Light(
        vd.vec3(2.0, 3.0, 4.0),
        vd.vec3(1.0, 0.6, 0.25),
        vd.f32(8.0),
    )
    point = vd.vec3(0.0, 0.0, 0.0)
    host_value = light.contribution(point)

    identity = vd.mat2(1.0, 0.0, 0.0, 1.0)
    transformed = vd.matmul(identity, vd.vec2(host_value[0], host_value[1]))
    reflected = vd.reflect(vd.vec3(0.0, -1.0, 0.0), vd.vec3(0.0, 1.0, 0.0))
    ...
    try:
        light.device_lambert(point, vd.vec3(0.0, 1.0, 0.0))
