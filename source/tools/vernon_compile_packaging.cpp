...
VernonStatus packageCompileResult(VernonCompilerContext *context,
  ...
  for (size_t index = 0; index < artifactCount; ++index) {
    if (options.outputDirectory) {
      ...
      std::filesystem::path path =
          *options.outputDirectory / std::string(name.data, name.size);
  ...
  if (options.shaderBundlePath) {
    ...
    if (error || !std::filesystem::is_directory(*options.shaderBundlePath)) {
      standardError << "cannot create bundle directory "
                    << options.shaderBundlePath->string() << '\n';
      return VERNON_STATUS_INTERNAL_ERROR;
    }
    for (size_t index = 0; index < artifactCount; ++index) {
      VernonStringView name = vernonCompileResultGetArtifactName(result, index);
      ...
      std::filesystem::path artifactPath =
          *options.shaderBundlePath / std::string(name.data, name.size);
      std::ofstream artifactOutput(artifactPath, std::ios::binary);
      writeView(artifactOutput, data);
      artifactOutput.close();
      if (!artifactOutput) {
        standardError << "cannot write bundle artifact "
                      << artifactPath.string() << '\n';
        return VERNON_STATUS_INTERNAL_ERROR;
      }
    }
    std::filesystem::path manifestPath =
        *options.shaderBundlePath / "shader.json";
    ...
    if (error || !std::filesystem::is_directory(*options.computeBundlePath)) {
      standardError << "cannot create compute bundle directory "
                    << options.computeBundlePath->string() << '\n';
      return VERNON_STATUS_INTERNAL_ERROR;
    }
    ...
    std::ofstream artifactOutput(*options.computeBundlePath / artifactFilename,
                                 std::ios::binary);
    artifactOutput.write(packagedArtifact.data(),
    ...
    manifest["schema_version"] =
        target == VERNON_TARGET_CPU
            ? (options.hostRuntimeBundle ? int64_t{2} : int64_t{3})
            : int64_t{1};
    ...
    if (target == VERNON_TARGET_CPU) {
      ...
      if (options.hostRuntimeBundle) {
        ...
        const llvm::Triple triple(
            llvm::Triple::normalize(options.targetTriple.value_or(
    ...
    manifest["artifact_format"] =
        target == VERNON_TARGET_CPU
            ? (options.hostRuntimeBundle ? "native_library"
                                         : "relocatable_object")
    ...
    std::ofstream manifestOutput(*options.computeBundlePath / "compute.json",
                                 std::ios::binary);
    std::string encoded;
    llvm::raw_string_ostream stream(encoded);
