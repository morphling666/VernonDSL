...
int main(int argc, char **argv) {
  ...
  if (!validateOnly) {
    ...
    if (packaging.shaderBundlePath.has_value() !=
        packaging.assetId.has_value()) {
    ...
    if (packaging.computeBundlePath &&
        (packaging.shaderBundlePath || packaging.outputDirectory)) {
      std::cerr << "--compute-bundle cannot be combined with --bundle or "
                   "--output-dir\n";
      return 2;
    }
    if (packaging.hostRuntimeBundle &&
        (!packaging.computeBundlePath || *target != VERNON_TARGET_CPU)) {
      std::cerr << "--host-runtime-bundle requires a CPU --compute-bundle\n";
      return 2;
    }
    if ((packaging.targetTriple || cpuName || cpuFeatures) &&
  ...
  } else if (glslVersion || packaging.targetTriple || cpuName || cpuFeatures) {
  ...
  if (status != VERNON_STATUS_OK) {
    ...
    status = vernon::tools::packageCompileResult(
        context, result, target.value_or(VERNON_TARGET_CPU), packaging,
        std::cout, std::cerr);
  }
