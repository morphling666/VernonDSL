...
std::wstring utf8ToWide(const char *value) {
    ...
    if (!MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, value, -1, result.data(), size))
        return {};
    return result;
}
#endif
...
bool PlatformLibrary::open(const char *path, std::string &error) {
    ...
    if (!handle_)
        error = "cannot load dynamic library '" + std::string(path) + "': " + windowsError(GetLastError());
#else
    dlerror();
    ...
    if (!handle_) {
        ...
        error =
            "cannot load dynamic library '" + std::string(path) + "': " + (message ? message : "unknown loader error");
    }
#endif
