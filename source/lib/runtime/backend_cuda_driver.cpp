...
#define VERNON_LOAD_CUDA(member, symbolName)                                   \
  ...
  memoryAllocate =
      reinterpret_cast<MemoryAllocate>(library.symbol("cuMemAlloc_v2"));
  if (!memoryAllocate)
    memoryAllocate =
        reinterpret_cast<MemoryAllocate>(library.symbol("cuMemAlloc"));
  ...
  copyHostToDevice =
      reinterpret_cast<CopyHostToDevice>(library.symbol("cuMemcpyHtoD_v2"));
  if (!copyHostToDevice)
    copyHostToDevice =
        reinterpret_cast<CopyHostToDevice>(library.symbol("cuMemcpyHtoD"));
  copyDeviceToHost =
      reinterpret_cast<CopyDeviceToHost>(library.symbol("cuMemcpyDtoH_v2"));
  if (!copyDeviceToHost)
    copyDeviceToHost =
        reinterpret_cast<CopyDeviceToHost>(library.symbol("cuMemcpyDtoH"));
  if (!memoryAllocate || !memoryFree || !copyHostToDevice ||
      !copyDeviceToHost) {
