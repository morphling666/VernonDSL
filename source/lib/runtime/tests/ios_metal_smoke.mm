...
bool testBuffers(VernonRhiDevice device) {
  ...
  for (VernonRhiMemoryClass memoryClass :
       {VERNON_RHI_MEMORY_DEVICE, VERNON_RHI_MEMORY_UPLOAD,
        VERNON_RHI_MEMORY_READBACK}) {
    ...
    descriptor.usage = VERNON_RHI_BUFFER_TRANSFER_SOURCE |
                       VERNON_RHI_BUFFER_TRANSFER_DESTINATION |
    ...
    const bool success =
        vernonRhiDeviceCreateBuffer(device, &descriptor, &buffer) ==
            VERNON_RHI_STATUS_OK &&
        vernonRhiDeviceUploadBuffer(device, buffer, 0, source.data(),
                                    sizeof(source)) == VERNON_RHI_STATUS_OK &&
        vernonRhiDeviceDownloadBuffer(device, buffer, 0, destination.data(),
                                      sizeof(destination)) ==
            VERNON_RHI_STATUS_OK &&
        destination == source;
...
bool testImageAndSampler(VernonRhiDevice device) {
  ...
  bool success = vernonRhiDeviceUploadImage(device, image, &upload, 1) ==
                     VERNON_RHI_STATUS_OK &&
                 vernonRhiDeviceGenerateImageMipmaps(device, image) ==
                     VERNON_RHI_STATUS_OK &&
                 vernonRhiDeviceDownloadImage(
                     device, image, &download, destination.data(),
                     destination.size()) == VERNON_RHI_STATUS_OK &&
                 destination == source;
  ...
  success =
      success && vernonRhiDeviceCreateSampler(device, &samplerDescriptor,
                                              &sampler) == VERNON_RHI_STATUS_OK;
  if (sampler.index != VERNON_RHI_INVALID_HANDLE_INDEX)
    success = vernonRhiDeviceDestroySampler(device, sampler) ==
                  VERNON_RHI_STATUS_OK &&
              success;
  success =
      vernonRhiDeviceDestroyImage(device, image) == VERNON_RHI_STATUS_OK &&
      success;
  return success;
}
