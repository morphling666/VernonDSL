...
llvm::Error invalidAbi(const llvm::Twine &message) {
...
llvm::Error defineCpuTextureSampleHelper(llvm::Module &module) {
  ...
  if (helper->arg_size() != 4 ||
      helper->getArg(0)->getType() != llvm::Type::getInt64Ty(context) ||
      helper->getArg(1)->getType() != llvm::Type::getInt64Ty(context) ||
      helper->getArg(2)->getType() != uvType ||
      helper->getArg(3)->getType() != pointerType ||
      helper->getReturnType() != resultType)
  ...
  builder.CreateCall(callbackType, sampleFunction,
                     {userData, helper->getArg(0),
                      builder.CreateExtractElement(uv, uint64_t{0}),
...
llvm::Error emitCpuAbiWrapper(llvm::Module &module,
                              const CpuAbiWrapperMetadata &metadata) {
  llvm::Function *function =
      module.getFunction(metadata.internalFunctionSymbol);
  if (!function)
    return invalidAbi("CPU ABI wrapper internal function '" +
                      metadata.internalFunctionSymbol + "' does not exist");
  ...
  llvm::Function *wrapper =
      llvm::Function::Create(wrapperType, llvm::GlobalValue::ExternalLinkage,
                             metadata.exportedWrapperSymbol, module);
  ...
  llvm::LoadInst *argumentsSize =
      builder.CreateLoad(llvm::Type::getInt64Ty(context), argumentsSizeAddress);
      ...
      metadata.argumentsSize == 0
          ...
          : builder.CreateAnd(
                builder.CreateICmpNE(
                    arguments, llvm::ConstantPointerNull::get(
                                   llvm::cast<llvm::PointerType>(pointerType))),
                builder.CreateICmpUGE(
                    argumentsSize,
                    llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),
                                           metadata.argumentsSize)));
  ...
  for (const CpuAbiArgumentPacking &packing : metadata.sourceArguments) {
    llvm::Value *address =
        builder.CreateGEP(llvm::Type::getInt8Ty(context), arguments,
                          llvm::ConstantInt::get(
                              llvm::Type::getInt64Ty(context), packing.offset));
    ...
    if (!offset || !extent || !stride)
      return invalidAbi("lowered CPU buffer descriptor index types are "
                        "incompatible");
    llvm::LoadInst *rawPointer =
        builder.CreateLoad(pointerType, address, "buffer");
