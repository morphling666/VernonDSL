...
convertStaticTensorToVector(Type type, std::optional<int64_t> elementLimit) {
...
struct ConstantPattern final : OpConversionPattern<arith::ConstantOp> {
  ...
  matchAndRewrite(arith::ConstantOp op, OpAdaptor,
    ...
    if (!elements)
      return failure();
    auto resultType = dyn_cast_if_present<VectorType>(
...
void addVernonSharedValueTypeConversions(
    TypeConverter &converter, std::optional<int64_t> staticTensorElementLimit) {
  converter.addConversion([](Type type) { return type; });
  converter.addConversion([staticTensorElementLimit](
                              RankedTensorType tensor) -> std::optional<Type> {
    FailureOr<VectorType> converted =
        convertStaticTensorToVector(tensor, staticTensorElementLimit);
    if (failed(converted))
      return std::nullopt;
    return *converted;
  });
...
void populateVernonSharedValuePatterns(TypeConverter &converter,
  ...
  patterns
      .add<ConstantPattern, FromElementsPattern, SplatPattern, ExtractPattern,
           ...
           ElementwisePattern<arith::SubIOp>,
