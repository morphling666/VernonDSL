...
struct VernonValidatePass
    : public PassWrapper<VernonValidatePass, OperationPass<ModuleOp>> {
  ...
  StringRef getDescription() const final {
    return "Validate Vernon shader stages and interface attributes";
  }
  ...
  void runOnOperation() override {
    ...
    for (func::FuncOp function : getOperation().getOps<func::FuncOp>()) {
      ...
      if (entryAttr && !isa<UnitAttr>(entryAttr)) {
        function.emitError()
            << "'" << kEntryAttrName << "' must be a unit attribute";
        invalid = true;
      }

      if (stageAttr && !entryAttr) {
        function.emitError()
            << "'" << kStageAttrName << "' requires '" << kEntryAttrName << "'";
        invalid = true;
      }
      if (entryAttr && !stageAttr) {
        function.emitError()
            << "'" << kEntryAttrName << "' requires '" << kStageAttrName << "'";
        invalid = true;
      }
      ...
      if (stage == ShaderStage::Compute) {
        ...
        if (!workgroup || workgroup.size() != 3 ||
            llvm::any_of(workgroup.asArrayRef(),
                         [](int32_t value) { return value <= 0; })) {
          function.emitError()
              << "compute entry requires '" << kWorkgroupSizeAttrName
              << "' as three positive i32 values";
          invalid = true;
        }
      } else if (workgroupAttr) {
        function.emitError() << "'" << kWorkgroupSizeAttrName
                             << "' is only valid on a compute entry";
        invalid = true;
      }
      ...
      auto validateInterface = [&](DictionaryAttr dictionary, Type type,
                                   unsigned index, bool isResult) {
        InterfaceAttrs attrs = parseInterfaceAttrs(dictionary);
        if (attrs.empty()) {
          if (entryAttr) {
            function.emitError() << (isResult ? "result #" : "argument #")
                                 << index << " of a Vernon entry must have '"
                                 << kInterfaceAttrName << "'";
            invalid = true;
          }
          return;
        ...
        if (isResult && kind != InterfaceKind::Output) {
          function.emitError() << "result #" << index
                               << " must use vernon.interface = \"output\"";
          invalid = true;
        }
        if (!isResult && kind == InterfaceKind::Output) {
          function.emitError() << "argument #" << index
                               << " cannot use vernon.interface = \"output\"";
          invalid = true;
        }
        ...
        if (attrs.location && attrs.builtin) {
          function.emitError()
              << (isResult ? "result #" : "argument #") << index
              << " cannot have both '" << kLocationAttrName << "' and '"
              << kBuiltinAttrName << "'";
          invalid = true;
        }

        bool isIo =
            kind == InterfaceKind::Input || kind == InterfaceKind::Output;
        bool isDescriptor =
            kind == InterfaceKind::Uniform || kind == InterfaceKind::Resource;
        ...
        auto parseNonNegative = [&](Attribute attr,
        ...
        std::optional<int64_t> descriptorSet =
            parseNonNegative(attrs.descriptorSet, kDescriptorSetAttrName);
        std::optional<int64_t> binding =
            parseNonNegative(attrs.binding, kBindingAttrName);

        if (kind == InterfaceKind::Resource &&
            (!attrs.descriptorSet || !attrs.binding)) {
          function.emitError()
              << (isResult ? "result #" : "argument #") << index << " "
              << stringifyInterfaceKind(kind) << " requires both '"
              << kDescriptorSetAttrName << "' and '" << kBindingAttrName << "'";
          invalid = true;
        }
        if (kind == InterfaceKind::Uniform &&
            static_cast<bool>(attrs.descriptorSet) !=
                static_cast<bool>(attrs.binding)) {
          function.emitError()
              << (isResult ? "result #" : "argument #") << index
              << " uniform must provide both '" << kDescriptorSetAttrName
              << "' and '" << kBindingAttrName << "', or neither";
          invalid = true;
        }
        ...
        if (descriptorSet && binding &&
            !bindings.insert({*descriptorSet, *binding}).second) {
          function.emitError()
              << (isResult ? "result #" : "argument #") << index
              << " conflicts with another interface at descriptor set "
        ...
        if (attrs.instanceDivisor) {
          ...
          if (isResult || *stage != ShaderStage::Vertex ||
              kind != InterfaceKind::Input) {
            function.emitError() << (isResult ? "result #" : "argument #")
                                 << index << " '" << kInstanceDivisorAttrName
                                 << "' is only valid on vertex inputs";
            invalid = true;
          }
        }
        ...
        std::string slot = location ? ("location:" + std::to_string(*location))
        ...
        if (!inserted) {
          function.emitError()
              << (isResult ? "result #" : "argument #") << index
              << " conflicts with "
              << (kind == InterfaceKind::Input ? "input argument #"
                                               : "output result #")
      ...
      for (unsigned index = 0; index < function.getNumArguments(); ++index)
        validateInterface(function.getArgAttrDict(index),
                          function.getArgumentTypes()[index], index,
                          /*isResult=*/false);
      for (unsigned index = 0; index < function.getNumResults(); ++index)
        validateInterface(function.getResultAttrDict(index),
                          function.getResultTypes()[index], index,
                          /*isResult=*/true);
    }
    ...
    if (hasVertexEntry && hasFragmentEntry) {
      for (const LocationEndpoint &fragmentInput : fragmentInputs) {
        ...
        auto matchingLocation =
            llvm::find_if(vertexOutputs, [&](const LocationEndpoint &output) {
              return output.location == fragmentInput.location;
            });
        if (matchingLocation == vertexOutputs.end()) {
          fragmentFunction.emitError()
              << "fragment input #" << fragmentInput.index << " at location "
...
std::unique_ptr<Pass> createVernonValidatePass() {
  return std::make_unique<VernonValidatePass>();
}
