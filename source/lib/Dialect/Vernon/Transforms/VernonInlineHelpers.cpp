...
struct VernonInlineHelpersPass final : PassWrapper<VernonInlineHelpersPass, OperationPass<ModuleOp>> {
    ...
    StringRef getDescription() const final { return "Inline shared Vernon helper functions into their callers"; }

    void runOnOperation() override {
        ModuleOp module = getOperation();
        auto eraseHelpers = [&] {
            for (func::FuncOp function : llvm::make_early_inc_range(module.getOps<func::FuncOp>()))
                if (!function->hasAttr("vernon.entry") && !function->hasAttr("vernon_program.graph"))
                    function.erase();
        };
        ...
        for (unsigned iteration = 0; iteration < maxIterations; ++iteration) {
            ...
            for (func::CallOp call : calls) {
                ...
                auto returnOp = dyn_cast<func::ReturnOp>(callee.front().getTerminator());
                if (!returnOp) {
        ...
        if (!remainingCalls.empty()) {
            remainingCalls.front().emitError("helper inlining did not converge; recursive calls are forbidden");
            signalPassFailure();
            return;
        }
        eraseHelpers();
...
std::unique_ptr<Pass> createVernonInlineHelpersPass() { return std::make_unique<VernonInlineHelpersPass>(); }

void registerVernonInlineHelpersPass() { PassRegistration<VernonInlineHelpersPass>(); }
