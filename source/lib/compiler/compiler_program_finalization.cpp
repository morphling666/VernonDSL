...
bool buildCanonicalComputeProgram(const llvm::json::Object &execution,
    ...
    if (!rawValues || !rawGraphs || rawGraphs->size() != 1 || !rawSignature) {
        error = "canonical compute finalization requires exactly one forward graph and signature";
        return false;
    }
    const llvm::json::Object *graph = (*rawGraphs)[0].getAsObject();
    const llvm::json::Array *nodes = graph ? graph->getArray("nodes") : nullptr;
    const llvm::json::Array *argumentIds = graph ? graph->getArray("arguments") : nullptr;
    const llvm::json::Array *resultIds = graph ? graph->getArray("results") : nullptr;
    if (!graph || graph->getString("direction") != "forward" || !nodes || nodes->empty() || !argumentIds ||
        !resultIds) {
        error = "canonical compute finalization requires one non-empty forward compute graph";
        return false;
    }
    if (nodes->size() != compiledStages.size()) {
        error = "compiled stages do not exactly cover canonical compute nodes";
        return false;
    }

    std::map<int64_t, size_t> argumentSlots;
    for (size_t slot = 0; slot < argumentIds->size(); ++slot) {
        std::optional<int64_t> id = (*argumentIds)[slot].getAsInteger();
        if (!id || !argumentSlots.emplace(*id, slot).second) {
            error = "canonical compute graph has invalid argument ids";
            return false;
        }
    }
    std::map<int64_t, int64_t> producerByValue;
    ...
    for (const llvm::json::Value &nodeValue : *nodes) {
        ...
        for (const llvm::json::Value &value : *nodeResults) {
            ...
            if (!id || !valueById(*rawValues, *id) ||
                !producerByValue.emplace(*id, node->getInteger("id").value_or(-1)).second) {
    ...
    for (size_t expectedId = 0; expectedId < rawValues->size(); ++expectedId) {
        ...
        if (auto argument = argumentSlots.find(static_cast<int64_t>(expectedId)); argument != argumentSlots.end())
            origin = llvm::json::Object{{"tag", "argument"},
                                        {"graph", graph->getString("name").value_or("forward").str()},
                                        {"slot", static_cast<int64_t>(argument->second)}};
    ...
    llvm::json::Array canonicalNodes;
    for (const llvm::json::Value &nodeValue : *nodes) {
        const llvm::json::Object *node = nodeValue.getAsObject();
        ...
        if (node->getString("kind") == "render") {
                    ...
                    for (size_t ordinal = 0; ordinal < rows->size(); ++ordinal) {
                        ...
                        if (implicit == "resolution") {
                            ...
                            abiBindings.emplace_back(llvm::json::Object{
                                {"semantic", "value"}, {"carrier", valueCarrier("value_slot", nextSlot++, *layout)}});
                                ...
                                for (const llvm::json::Value &leafValue : *row->getArray("attribute_leaves"))
                                    if (const llvm::json::Object *leaf = leafValue.getAsObject())
                                        vertexInputs.emplace_back(llvm::json::Object{
                                            ...
                                            {"format", vertexFormat(leaf->getString("dtype").value_or("f32"),
            ...
            llvm::json::Object operation{
                ...
                {"state", llvm::json::Object{{"raster", llvm::json::Object{{"front_face", "counter_clockwise"},
                                                                           {"cull_mode", "none"},
                                                                           {"fill_mode", "fill"}}},
                                             {"depth_stencil",
                                              llvm::json::Object{{"depth_test", hasDepth},
                                                                 {"depth_write", hasDepth},
                                                                 {"depth_compare", hasDepth ? "less" : "always"},
                                                                 {"stencil_test", false}}},
                                             {"multisample", llvm::json::Object{{"sample_mask", int64_t{4294967295ULL}},
                                                                                {"alpha_to_coverage", false}}},
                                             {"blend", llvm::json::Array()},
                                             {"viewport", nullptr},
                                             {"scissor", nullptr}}},
                {"draw",
                 llvm::json::Object{{"tag", "direct"}, {"vertex_count", vertexCount}, {"instance_count", int64_t{1}}}},
            };
        ...
        const auto appendEndpoint = [&](const llvm::json::Object &row, llvm::StringRef interfaceKind,
            ...
            if (builtin) {
                ...
                abi.emplace_back(llvm::json::Object{{"semantic", "value"},
                                                    {"carrier", valueCarrier("constant_region", nextSlot++, *layout)}});
                endpoints.emplace_back(llvm::json::Object{
                    {"tag", "value"},
                    {"module", "compute"},
                    {"interface", "system_value"},
            ...
            if (!opaqueMatches && !byteValueMatches) {
                error =
                    "compiled compute endpoint layout/type/shape does not match logical value '" + source->str() + "'";
                return false;
            }
        ...
        stageContracts[requestId] = std::move(stageContract);
        if (!implementationEndpoints.empty())
            targetImplementations[requestId] = llvm::json::Object{{"endpoints", std::move(implementationEndpoints)}};
    ...
    canonicalGraphs.emplace_back(
        llvm::json::Object{{"name", graph->getString("name").value_or("forward").str()},
                           {"direction", graph->getString("direction").value_or("forward").str()},
                           {"inputs", std::move(graphInputs)},
                           {"captures", llvm::json::Array()},
                           {"outputs", std::move(graphOutputs)},
                           {"nodes", std::move(canonicalNodes)}});
    ...
    program = llvm::json::Object{
        ...
        {"signature", llvm::json::Object{{"inputs", signatureRows("inputs", false)},
                                         ...
                                         {"cotangents", llvm::json::Array()},
                                         {"gradients", llvm::json::Array()}}},
    };
    return true;
}
