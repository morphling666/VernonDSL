...
std::unique_ptr<VernonStageExecutable> structuredPipeline(std::vector<uint64_t> outerShape = {}) {
    ...
    pipeline->bindingProjection.parameters.push_back(std::move(parameter));
    return pipeline;
}
...
TEST(StructuredValueBinding, PacksNestedFieldAndTupleElementPaths) {
    auto pipeline = structuredPipeline();
    auto &layout = pipeline->bindingProjection.parameters[0].elementLayout;
    layout.byteSize = 8;
    layout.alignment = 4;
...
TEST(StructuredValueBinding, PacksTensorFromStructureOfArrays) {
    auto pipeline = structuredPipeline({2, 2});
...
TEST(StructuredValueBinding, PacksTensorFromLogicalElementCallback) {
    ...
    StageInvocationBuilder builder(pipeline.get());
    builder.bindTensor(
        "value", {2, 2}, [](StageInvocationBuilder::ElementWriter &element, const std::vector<uint64_t> &index) {
