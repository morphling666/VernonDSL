...
#endif
#ifndef VERNON_DIRECTX_AUTODIFF_PIPELINE_BUNDLE
#error VERNON_DIRECTX_AUTODIFF_PIPELINE_BUNDLE must name the cooked autodiff pipeline bundle
#endif
...
TEST(RuntimeDirectX12Pipeline, ExecutesReusableCookedGpuPullbackAfterPipelineDestruction) {
    const std::filesystem::path manifestPath = VERNON_DIRECTX_AUTODIFF_PIPELINE_BUNDLE;
    std::ifstream input(manifestPath, std::ios::binary);
    const std::string bundle((std::istreambuf_iterator<char>(input)), std::istreambuf_iterator<char>());
    ASSERT_FALSE(bundle.empty());
