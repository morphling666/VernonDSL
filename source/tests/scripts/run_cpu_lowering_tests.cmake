...
function(
    ...
    if(expect_failure)
        if(result EQUAL 0)
            message(FATAL_ERROR "${name}: expected vernon-opt to fail\nOutput:\n${output}")
        endif()
    elseif(
        NOT
        result
        EQUAL
        0)
        message(FATAL_ERROR "${name}: vernon-opt failed with ${result}\nOutput:\n${output}")
    endif()
...
function(
    ...
    if(position EQUAL -1)
        message(FATAL_ERROR "${case_name}: missing expected text '${needle}'\nOutput:\n${text}")
    endif()
endfunction()
...
function(
    ...
    if(NOT
       ...
       EQUAL
       -1)
        message(FATAL_ERROR "${case_name}: found forbidden text '${needle}'\nOutput:\n${text}")
    endif()
endfunction()
...
run_case(
    ...
    FALSE)
require_text("texture helper" "${TEXTURE_OUTPUT}" "func.func private @__vernon_cpu_texture_sample")
require_text("texture helper" "${TEXTURE_OUTPUT}" "!llvm.ptr")
reject_text("texture helper" "${TEXTURE_OUTPUT}" "vernon.cpu.requires_texture_callbacks")
require_text("texture helper" "${TEXTURE_OUTPUT}" "call @__vernon_cpu_texture_sample")
...
run_case(
    ...
    "--vernon-lower-cpu-tensors"
    TRUE)
require_text("illegal operation diagnostic" "${ILLEGAL_OUTPUT}"
             "unknown Vernon intrinsic 'unimplemented_cpu_operation'")
require_text("illegal operation diagnostic" "${ILLEGAL_OUTPUT}" "in CPU entry 'unsupported_intrinsic'")
