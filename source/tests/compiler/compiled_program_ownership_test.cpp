...
TEST(CompiledProgramOwnership, OutlivesCompilerContext) {
  ...
  VernonCompileResult *program = vernonCompilerCompileMlirWithOptions(
      compiler, module.data(), module.size(), VERNON_TARGET_CPU, &options);
  ASSERT_TRUE(program);
  ASSERT_TRUE(vernonCompileResultGetStatus(program) == VERNON_STATUS_OK);
  ASSERT_TRUE(vernon::test::contains(vernonCompileResultGetReflection(program),
                                     R"json("target":"cpu")json"));
  ASSERT_TRUE(vernon::test::contains(vernonCompileResultGetReflection(program),
                                     R"json("cpu":"generic")json"));
  ASSERT_TRUE(vernon::test::contains(vernonCompileResultGetReflection(program),
                                     R"json("target_triple":)json"));

  VernonCpuEntryPoint entry =
      vernonCompileResultGetCpuEntry(program, "add_vectors", 11);
  ASSERT_TRUE(entry);
  vernonCompilerDestroy(compiler);

  float arguments[8] = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f};
  float results[4]{};
  VernonCpuInvocation invocation{arguments, sizeof(arguments), results,
                                 sizeof(results), nullptr};
